...
def test_serverless_version():
    with patch('bentoml.deployment.serverless.invoke_serverless_command', return_value=['1.40.0']):
        result = check_serverless_compatiable_version()
        assert result == True

    with patch('bentoml.deployment.serverless.invoke_serverless_command', return_value=['1.20.0']):
